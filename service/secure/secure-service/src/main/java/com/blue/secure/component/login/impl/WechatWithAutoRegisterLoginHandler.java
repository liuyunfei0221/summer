...
import com.blue.secure.remote.consumer.RpcWechatServiceConsumer;
...
import static com.blue.base.constant.secure.LoginType.WECHAT_AUTO_REGISTER;
                        ...
                        credentialOpt.map(credential -> {
                                    ...
                                    return memberService.selectMemberBasicInfoMonoById(credential.getMemberId())
                                            .flatMap(mbi -> {
                                                ...
                                                return secureService.generateAuthMono(mbi.getId(), WECHAT_AUTO_REGISTER.identity, loginParam.getDeviceType().intern());
                                            });
                                })
                                .orElseGet(() -> {
                                    ...
                                    return just(roleService.getDefaultRole().getId())
                                            .flatMap(roleId -> just(autoRegisterService.autoRegisterMemberInfo(CREDENTIALS_GENERATOR.apply(phone), roleId))
                                                    .flatMap(mbi -> secureService.generateAuthMono(mbi.getId(), roleId, WECHAT_AUTO_REGISTER.identity, loginParam.getDeviceType().intern())));
                                })
                )
                .flatMap(ma ->
