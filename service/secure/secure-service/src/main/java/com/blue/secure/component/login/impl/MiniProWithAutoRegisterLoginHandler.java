...
import com.blue.secure.remote.consumer.RpcMiniProServiceConsumer;
...
import static com.blue.base.constant.secure.LoginType.MINI_PRO_AUTO_REGISTER;
...
public class MiniProWithAutoRegisterLoginHandler implements LoginHandler {
    ...
    public MiniProWithAutoRegisterLoginHandler(RpcMiniProServiceConsumer rpcMiniProServiceConsumer, AutoRegisterService autoRegisterService,
                        ...
                        credentialOpt.map(credential -> {
                                    ...
                                    return memberService.selectMemberBasicInfoMonoById(credential.getMemberId())
                                            .flatMap(mbi -> {
                                                ...
                                                return secureService.generateAuthMono(mbi.getId(), MINI_PRO_AUTO_REGISTER.identity, loginParam.getDeviceType().intern());
                                            });
                                })
                                .orElseGet(() -> {
                                    ...
                                    return just(roleService.getDefaultRole().getId())
                                            .flatMap(roleId -> just(autoRegisterService.autoRegisterMemberInfo(CREDENTIALS_GENERATOR.apply(phone), roleId))
                                                    .flatMap(mbi -> secureService.generateAuthMono(mbi.getId(), roleId, MINI_PRO_AUTO_REGISTER.identity, loginParam.getDeviceType().intern())));
                                })
                )
                .flatMap(ma ->
