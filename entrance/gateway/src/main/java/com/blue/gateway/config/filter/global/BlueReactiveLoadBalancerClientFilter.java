...
public class BlueReactiveLoadBalancerClientFilter implements GlobalFilter, Ordered {
    ...
    private static final Supplier<BlueException> NOT_FOUND_EXP_SUP = () -> new BlueException(NOT_FOUND.status, NOT_FOUND.code, NOT_FOUND.message, null);
            ...
            return choose(exchange)
                    .doOnNext(response -> {
                        if (response.hasServer()) {
                            ...
                            throw new BlueException(NOT_FOUND.status, NOT_FOUND.code, NOT_FOUND.message, null);
                        }
