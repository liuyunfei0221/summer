...
public class ShineServiceImpl implements ShineService {
    ...
    private static final Function<ShineCondition, SortOptions> SORT_PROCESSOR = c -> {
        ...
        return process(
                sortAttribute.equals(ShineSortAttribute.ID.column) ?
                        singletonList(new SortElement(sortAttribute, sortType))
                        :
                        Stream.of(sortAttribute, ShineSortAttribute.ID.column)
    ...
    public Mono<ShineInfo> insertShine(ShineInsertParam shineInsertParam, Long memberId) {
        LOGGER.info("shineInsertParam = {}, memberId = {}", shineInsertParam, memberId);
        if (isNull(shineInsertParam))
            throw new BlueException(EMPTY_PARAM);
