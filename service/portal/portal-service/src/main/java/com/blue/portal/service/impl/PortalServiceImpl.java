...
public class PortalServiceImpl implements PortalService {
     ...
     */
    private final Function<Integer, List<BulletinInfo>> DB_PORTAL_GETTER = type -> {
        List<Bulletin> bulletins = bulletinService.selectAllPortalBulletinByType(type);
     ...
     */
    private final BiConsumer<Integer, List<BulletinInfo>> REDIS_PORTAL_SETTER = (type, list) -> {
        if (isEmpty(list))
            return;
     ...
     */
    private final Function<Integer, Mono<List<BulletinInfo>>> LOCAL_CACHE_PORTAL_FUNC = type -> {
        if (isNull(type))
    ...
    public void invalidBulletinInfosCache() {
        of(BulletinType.values())
                .forEach(type -> {
                    stringRedisTemplate.delete(BULLETIN_CACHE_KEY_GENERATOR.apply(type.identity));
                    LOCAL_CACHE.invalidate(type.identity);
                });
    }
